# leave-one-out cross-validation with FC metrics
...
methods <- list("plm", "aec", "plv")

# frequency bands of FC metrics
fbands <- list("delta", "theta", "alpha", "beta", "gamma")

datafolder <- "../../../data/"
stat_folder <- paste0(datafolder, "statistic/connectivity/HCP/Restin/", space, "/") # FC data folder
...
for (method in methods) {
  for (fband in fbands) {
    ...
    savepath_acc_data <- paste0(savepath_acc_fold, "connectivity/")
    ...
    #### load data
    method_band_folder <- paste0(stat_folder, method, "/", fband, "/")
